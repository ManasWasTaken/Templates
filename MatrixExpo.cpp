...
using Z = mint<>;
vector<vector<Z>> mul(vector<vector<Z>> A,vector<vector<Z>> B){
    //A*B
    int n=A.size();
    vector<vector<Z>> res(n,vector<Z> (n,Z(0)));
    for(int i=0;i<n;i++){
        for(int j=0;j<n;j++){
            for(int k=0;k<n;k++){
                Z add=A[i][j]*B[j][k];
...
vector<vector<Z>> matrix_expo(vector<vector<Z>> A,int k){
    int n=A.size();
    vector<vector<Z>> res(n,vector<Z> (n,Z(0)));
    vector<vector<Z>> base=A;
    for(int i=0;i<n;i++) res[i][i]=Z(1);
    while(k){
        if(k%2){
            res=mul(res,base);
        }
...
//matrix expo with mint, this can be made better
void solve(){
